...
class TransporterAgent:
    ...
    def validate(self, dataset, writer=None):  # pylint: disable=unused-argument
        ...
        for i in range(n_iter):
            ...
            source_translation_error += np.linalg.norm(
                pred_action["pose0"][0][:2] - gt_act["pose0"][0][:2]
            )
            goal_translation_error += np.linalg.norm(
                pred_action["pose1"][0][:2] - gt_act["pose1"][0][:2]
            )
        ...
        with writer.as_default():
            ...
            sc(
                "test_loss/source_translation_error",
                source_translation_error,
                self.total_steps,
            )
            sc(
                "test_loss/goal_translation_error",
                goal_translation_error,
                self.total_steps,
            )
