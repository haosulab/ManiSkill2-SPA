...
from ravens.utils import maniskill as maniskill_utils
...
class TransporterAgent:
  ...
  def __init__(self, name, task, root_dir, n_rotations=36):
    ...
    self.mani_skill = True
    if self.mani_skill:
      # For maniskill bounds/pix_size
  ...
  def get_image(self, obs):
    ...
    if self.mani_skill:
      cmap, hmap = maniskill_utils.get_fused_image(obs['color'], obs['depth'], obs['extrinsics'], obs['intrinsics'], self.pix_size)
    else:
  ...
  def train(self, dataset, writer=None):
    ...
    img, p0, p0_theta, p1, p1_theta, _ = self.get_sample(dataset)
  ...
  def validate(self, dataset, writer=None):  # pylint: disable=unused-argument
    ...
    for i in range(n_iter):
      ...
      pred_action = self._act(img)
      
      
      source_translation_error += np.linalg.norm(pred_action['pose0'][0][:2] - gt_act['pose0'][0][:2])
      goal_translation_error += np.linalg.norm(pred_action['pose1'][0][:2] - gt_act['pose1'][0][:2])
      goal_rot_error += np.linalg.norm(pred_action['pose1'][1] - gt_act['pose1'][1])
    ...
    with writer.as_default():
      ...
      sc('test_loss/source_translation_error', source_translation_error, self.total_steps)
      sc('test_loss/goal_translation_error', goal_translation_error, self.total_steps)
      sc('test_loss/goal_rot_error', goal_rot_error, self.total_steps)
    print(f'Validation Loss: {loss0:.4f} {loss1:.4f} - s {source_translation_error:.4f} - g {goal_translation_error:.4f} - r {goal_rot_error:.4f}')
...
class OriginalTransporterAgent(TransporterAgent):
  ...
  def __init__(self, name, task, root_dir, n_rotations=36):
    super().__init__(name, task, root_dir=root_dir, n_rotations=n_rotations)
